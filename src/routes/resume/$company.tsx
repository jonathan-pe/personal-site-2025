...
function RouteComponent() {
  ...
  // Generate structured data for the job experience
  const jobStructuredData = {
    ...
    endDate: job.endDate === 'Present' ? new Date().toISOString() : job.endDate,
    skills: job.techUsed,
    description: job.accomplishments.join('. '),
  }
  ...
  return (
    <>
      <SEOHead
        ...
        description={`${job.role} experience at ${job.companyName} (${job.startDate} - ${
          job.endDate
        }). Key technologies: ${job.techUsed.slice(0, 5).join(', ')}. ${job.accomplishments[0]}`}
        keywords={[
